...
static void runtime_error(const char *format, ...)
{
    ...
    for (int i = vm.frame_count - 1; i >= 0; i--) {
        ...
        fprintf(stderr, "[line %d] in ", get_line(&function->chunk, instruction));
...
static bool call(ObjClosure *closure, int arg_count)
{
    ...
    frame->slots = vm.stack_top - arg_count - 1; // Setting the frame window on the stack.
...
static bool call_value(Value callee, int arg_count)
{
    if (IS_OBJ(callee)) {
        switch (OBJ_TYPE(callee)) {
            ...
            case OBJ_NATIVE: {
                ObjNative *obj_native = AS_NATIVE(callee);
                if (obj_native->arity != arg_count) {
                    runtime_error("Expected %d arguments but got %d.", obj_native->arity, arg_count);
                    return false;
                }
        ...
        switch (instruction) {
            ...
            case OP_GET_GLOBAL: {
                ...
                if (!table_get(&vm.globals, name, &value)) {
                    runtime_error("Undefined varible '%s'.", name->chars);
                    return INTERPRET_RUNTIME_ERROR;
                }
...
InterpretResult interpret(const char *source)
{
    ...
    push(OBJ_VAL(function)); // Push the function in the stack as if it is the main function.
    ObjClosure *closure = new_closure(function);
    pop();
    push(OBJ_VAL(closure));
